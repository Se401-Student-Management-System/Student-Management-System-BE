...
public class CashierController {
    ...
    public ResponseEntity<List<PaymentRecordDTO>> exportPaymentRecords(
            @RequestParam(required = false) String academicYear,
            @RequestParam(required = false) String status) {
        try {
            // Kiểm tra trạng thái hợp lệ
            if (status != null && !status.equals("Đã thanh toán") && !status.equals("Chưa thanh toán") && !status.equals("Thanh toán 1 phần")) {
                throw new IllegalArgumentException("Trạng thái không hợp lệ: " + status);
            }
            PaymentCommand exportCommand = new ExportPaymentRecordCommand(paymentReceiptService, academicYear, status);
            invoker.addCommand(exportCommand);
            invoker.runCommands();

            List<PaymentRecordDTO> records = ((ExportPaymentRecordCommand) exportCommand).getResult();
